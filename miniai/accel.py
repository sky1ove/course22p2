...
# %% auto 0
__all__ = ['MixedPrecision', 'AccelerateCB', 'set_fabric', 'FabricCB', 'MultDL']
...
def set_fabric(precision="16-mixed", **kwargs):
    fabric=Fabric(precision=precision, **kwargs) # e.g., precision="bf16-true"
    fabric.launch()
...
class FabricCB(TrainCB):
    ...
    2. set_fabric() in the beginning of the code
    
    3. add fabric.setup_dataloaders(dl) to wrap dataloader
    
    """
    order = DeviceCB.order+10
    def before_fit(self,learn): learn.model,learn.opt = fabric.setup(learn.model,learn.opt)
    def backward(self, learn): fabric.backward(learn.loss)

# %% ../nbs/17_DDPM_v2.ipynb 63
class MultDL:
